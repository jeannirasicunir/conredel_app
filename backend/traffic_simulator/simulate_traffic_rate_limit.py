...
# Simulation settings
ERROR_RATE = 0.1  # 10% chance of simulating errors

# Rate limit testing settings
BURST_MODE = True  # Enable burst mode for rate limit testing
BURST_SIZE = 20  # Number of requests to send in a burst
MIN_DELAY = 0.001  # Minimal delay between requests (1ms)
MAX_DELAY = 0.01  # Maximum delay between requests (10ms)
...
client = httpx.Client(
    ...
    http2=False  # Disable HTTP/2 to test if it's causing issues
)
...
print(f"Starting traffic simulator (httpx version)... Press Ctrl+C to stop")
...
try:
    while True:
        try:
            if BURST_MODE:
                # In burst mode, send multiple requests quickly to the same endpoint
                for _ in range(BURST_SIZE):
                    is_error = random.random() < ERROR_RATE
                    choice = "GET_LIST"  # Focus on one endpoint for rate limit testing
                    current_headers = HEADERS  # Always use valid auth in burst mode
            
            # Print request timing details
            print(f"\n[DEBUG] Making {choice} request")
            ...
            if choice == "GET_LIST":
                ...
                resp = client.get(BASE, headers=current_headers, params=params)
                ...
                resp = client.post(BASE, json=data, headers=current_headers)
                ...
                if task_id:
                    ...
                    if choice == "GET_DETAIL":
                        resp = client.get(f"{BASE}{task_id}/", headers=current_headers)
                    elif choice == "PUT":
                        data = get_random_task_data()
                        if is_error:
                            data['priority'] = 999
                            data['status'] = 'invalid_status'
                        resp = client.put(f"{BASE}{task_id}/", json=data, headers=current_headers)
                    else:  # DELETE
                        resp = client.delete(f"{BASE}{task_id}/", headers=current_headers)
                else:
            ...
            if request_time > 1.0:  # More than 1 second
                time_color = RED
            elif request_time > 0.7:  # More than 700ms
                time_color = YELLOW
            else:
                time_color = GREEN
        ...
        # Shorter delay between requests to trigger rate limit
        if BURST_MODE:
